# PowerShell script to clean up locked build directories
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "   ⚠ Clean completed with warnings (some files may still be locked)" -ForegroundColor Yellow
    }
} catch {
    Write-Host "   ✗ Clean failed: $_" -ForegroundColor Red
    Write-Host "`n   Alternative: Manually delete the build directory:" -ForegroundColor Yellow
    Write-Host "   Remove-Item -Path 'app\build' -Recurse -Force" -ForegroundColor Cyan
}
